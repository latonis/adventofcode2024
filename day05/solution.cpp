...
std::vector<int> fix_layout(std::vector<int> page,
                            std::unordered_map<int, std::set<int>> order_map) {
    while (not valid_layout(page, order_map)) {
        for (auto o : order_map) {
            int i1 = index_of(page, o.first);
            for (auto const& s : o.second) {
                int i2 = index_of(page, s);
                ...
                if (i2 > i1) {
                    while (i2 > i1) {
                        std::swap(page[i2], page[i2 - 1]);
                        i2 = index_of(page, s);
                        i1 = index_of(page, o.first);
                    }
                }
            }
        }
...
int part_two() {
    ...
    int c = 0;
    for (const auto& page : pages) {
        if (not valid_layout(page, order_map)) {
            std::vector<int> n_vec = fix_layout(page, order_map);
            c += n_vec[n_vec.size() / 2];
        }
    }
    std::cout << "Total: " << c << std::endl;
    return 0;
